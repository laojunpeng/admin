  ...
  <div class="common-layout-container">
    <Row class="layout-content">
      ...
      <card>
        ...
        <Tabs type="card">
          <TabPane label="用户详情" icon="person">
            <customer ref="customer" :customerForm="customerForm" :rules="ruleValidate"
                      :orderState="orderState"></customer>
          </TabPane>
          <template v-if="orderState>1">
            <TabPane label="机器详情" icon="social-android">
              <robot v-for="(item,index) in customerForm.machines" :repairIndex="index" :repair="customerForm.repairs" :machine="customerForm.machines[index]"
                     :key="index"></robot>
            </TabPane>
          </template>
        </Tabs>
